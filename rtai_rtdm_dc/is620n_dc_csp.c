 ...
 *  本例演示正转3圈反转3圈,IS620N 20位增量型转1圈绝对位置值增加1048576
 *  $gcc -o is620n_dc_csp is620n_dc_csp.c -I/opt/etherlab/include -I/opt/rtai-5.1_v4.9.80/include -L/opt/rtai-5.1_v4.9.80/lib -L/opt/etherlab/lib -llxrt -lrtdm -lethercat -lm -lpthread -Wl,--rpath=/opt/rtai-5.1_v4.9.80/lib -Wl,--rpath=/opt/etherlab/lib
 *****************************************************************************/
...
void my_cyclic(void)
{
    ...
    uint32_t target_position = 0;int once=0;
	...
	temp[0]=EC_READ_U16(domain1_pd + offset.status_word_6041_0);
        ...
        if(!once++){
            for(i=0;i<N;i++)
               positions[i]=temp[1]+(1-cos(pi*i/N))/2*(M*1048576);
	    i=0;
        }
	//printf("\r%6f    \t    ",((float)temp[1]/1000) );
        ...
        /*if(servooff==1){//servo off
            ...
            EC_WRITE_S8(domain1_pd+offset.modes_operation_6060_0, 0x08);//csv mode
            EC_WRITE_U16(domain1_pd+offset.control_word_6040_0, 0x000f);
            //printf("3.state = %x\n",temp[0]);
        }
        else if( (temp[0]&0x06f) == 0x027){//600 800 //temp[0]=0x237
	        if(!dir){
                    EC_WRITE_S32(domain1_pd+offset.target_position_607a_0,positions[i]);
	            if(++i==N){
                       dir=1;
		       i=N-1;
                    }
